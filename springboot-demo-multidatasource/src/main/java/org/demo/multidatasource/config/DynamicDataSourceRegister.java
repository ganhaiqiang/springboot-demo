...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
	...
	private void initDefaultDataSource(Environment env) {
		...
		dsMap.put("url", env.getProperty("spring.datasource.hikari.jdbc-url"));
	...
	private void initslaveDataSources(Environment env) {
		...
		for (String dsPrefix : dsPrefixs.split(",")) {
			...
			dsMap.put("url", env.getProperty("spring.datasource." + dsPrefix + ".jdbc-url"));
			...
			logger.debug("dsPrefix={}", dsPrefix);
	...
	public DataSource buildDataSource(Map<String, Object> dataSourceMap) {
		try {
			...
			// 自定义DataSource配置
			DataSourceBuilder<? extends DataSource> factory = DataSourceBuilder.create().driverClassName(driverClassName).url(url).username(username).password(password).type(dataSourceType);
